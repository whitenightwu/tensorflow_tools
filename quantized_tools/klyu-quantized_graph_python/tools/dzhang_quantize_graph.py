...
# 32bit to 8bit parameter
deepth_quantize_parameter_num = 0
input_filter_offset_num = 0
bias_add_min_max_num = 0
relu_num = 0;
quantized_multiplier_arry = []
right_shift_arry = []
offset_input_arry = []
offset_filter_arry = []
offset_output_arry = []
bias_add_offset_input1 = []
bias_add_offset_input2 = []
bias_add_multiplier_input1 = []
bias_add_shift_input1 = []
bias_add_multiplier_input2 = []
bias_add_shift_input2 = []
relu_arry = []
fc_bias_add_min = 0
fc_bias_add_max = 0
...
flags.DEFINE_float(
    ...
    "enough for experimentation purposes, since graphs quantized in this way "
    "would be very inaccurate.")
flags.DEFINE_string("parameter_file_name", "", """32bit to 8bit need parameter""")


def save_to_global_arry(global_arry, value_list):
    for value in value_list[1:-1]:
        global_arry.append(int(value))
...
def create_constant_node(name, value, dtype, shape=None,value2 = 1):
  node = create_node("Const", name, [])

  if 'biases_input_filter_offset' in name:
      dtype = dtypes.int32
      set_attr_dtype(node, "dtype", dtype)
      global bias_add_min_max_num
      set_attr_tensor(node, "value", [
                                      bias_add_offset_input1[bias_add_min_max_num],
                                      bias_add_offset_input2[bias_add_min_max_num],
                                      bias_add_multiplier_input1[bias_add_min_max_num],
                                      bias_add_shift_input1[bias_add_min_max_num],
                                      bias_add_multiplier_input2[bias_add_min_max_num],
                                      bias_add_shift_input2[bias_add_min_max_num]
                                      ], dtype, [1,6])

      bias_add_min_max_num = bias_add_min_max_num + 1
      return node


  if '_input_filter_offset' in node.name :
      dtype = dtypes.int32
      set_attr_dtype(node, "dtype", dtype)
      global input_filter_offset_num
      set_attr_tensor(node, "value", [offset_input_arry[input_filter_offset_num],offset_filter_arry[input_filter_offset_num]], dtype, [1,2])
      input_filter_offset_num = input_filter_offset_num + 1
      return node


  

  if "deep_quantize_parameter" in node.name:
      dtype = dtypes.int32
      set_attr_dtype(node, "dtype", dtype)
      global deepth_quantize_parameter_num
      set_attr_tensor(node, "value", [
                                      quantized_multiplier_arry[deepth_quantize_parameter_num],
                                      right_shift_arry[deepth_quantize_parameter_num],
                                      offset_output_arry[deepth_quantize_parameter_num]], dtype, [1,3])
      deepth_quantize_parameter_num = deepth_quantize_parameter_num + 1
      return node
...
def quantize_weight_eightbit(input_node, quantization_mode):
  ...
  min_name = base_name + 'input_filter_offset'
  max_name = base_name + 'deep_quantize_parameter'
  ...
  min_node = create_constant_node(min_name, min_value, dtypes.float32,value2=max_value)
...
class GraphRewriter(object):
  ...
  def should_quantize_const(self, node):
    if "Relu_min_quantized" in node.name:
        return False
    if not self.state.output_node_stack:
      return False
    top = self.state.output_node_stack[-1]
  ...
  def eightbitize_nodes_recursively(self, current_node):
    ...
    for i, input_node_name in enumerate(current_node.input):
      ...
      if current_node.op in ("MatMul", "Conv2D", "BiasAdd", "MaxPool",
                             "AvgPool", "Relu", "Relu6",'DepthwiseConv2dNative',
  ...
  def add_eightbit_prologue_nodes(self, original_node):
    ...
    for original_input_name in original_node.input:
      #if "Relu_min_quantized" in original_input_name:
      #    input_names.append(original_input_name)
      #    continue
      quantize_input_name, min_output_name, max_output_name = (
          self.eightbitize_input_to_node(namespace_prefix, original_input_name,
                                         reshape_dims_name,
                                         reduction_dims_name))
      input_names.append(quantize_input_name)
      min_max_names_arry.append(min_output_name)
      min_max_names_arry.append(max_output_name)
          
    if original_node.op == "Relu":
        node_name = original_input_name + "_relu_min_quantized"
        node = create_node("Const", node_name, [])
        dtype = dtypes.int32
        set_attr_dtype(node, "dtype", dtype)
        global relu_num
        set_attr_tensor(node, "value", [ relu_arry[relu_num] ], dtype, [1,1])
        relu_num = relu_num + 1
    ...
    all_input_names.extend(min_max_names_arry)
        
        
    if original_node.op == "BiasAdd":
        # the node only useful in fc_Biass_Add
        node_name_min_max = original_input_name + "_output_min_max"
        node_min_max = create_node("Const", node_name_min_max, [])
        dtype = dtypes.float32
        set_attr_dtype(node_min_max, "dtype", dtype)
        global fc_bias_add_min
        global fc_bias_add_max
        set_attr_tensor(node_min_max, "value", [fc_bias_add_min,fc_bias_add_max], dtype, [1,2])
        self.add_output_graph_node(node_min_max)
        all_input_names.append(node_name_min_max)
  ...
  def eightbitize_mat_mul_node(self, original_node):
    ...
    set_attr_dtype(quantized_mat_mul_node, "Toutput", dtypes.quint8)
    ...
    #                                                 quantized_mat_mul_name)
    self.add_dequantize_result_node(quantized_mat_mul_name, original_node.name)
  ...
  def eightbitize_conv_node(self, original_node):
    ...
    set_attr_dtype(quantized_conv_node, "out_type", dtypes.quint8)
    self.add_output_graph_node(quantized_conv_node)
    #quantize_down_name = self.add_quantize_down_nodes(original_node,
    #                                                 quantized_conv_name)
    self.add_dequantize_result_node(quantized_conv_name, original_node.name)


   # MobileNet/conv_ds_2/depthwise_conv/depthwise_eightbit_quantized_depthwise
#quantized_multiplier:1526567680
#right_shift:5
#offset_output:103
  ...
  def eightbitize_depthwise_conv_node(self, original_node):
    """Replaces a Conv2D node with the eight bit equivalent sub-graph."""
    all_input_names = self.add_eightbit_prologue_nodes(original_node)
    quantized_conv_name = original_node.name + "_eightbit_quantized_depthwise"
    quantized_conv_node = create_node("QuantizedDepthwiseConv", quantized_conv_name,
                                      all_input_names)
    copy_attr(quantized_conv_node, "strides", original_node.attr["strides"])
    copy_attr(quantized_conv_node, "padding", original_node.attr["padding"])
    set_attr_dtype(quantized_conv_node, "Tinput", dtypes.quint8)
  ...
  def eightbitize_bias_add_node(self, original_node):
    ...
    set_attr_dtype(quantized_bias_add_node, "out_type", dtypes.quint8)
    self.add_output_graph_node(quantized_bias_add_node)
    #quantize_down_name = self.add_quantize_down_nodes(original_node,
    #                                                  quantized_bias_add_name)
    self.add_dequantize_result_node(quantized_bias_add_name, original_node.name)
  ...
  def remove_redundant_quantization(self, old_graph):
    ...
    for node in old_graph.node:
      ...
      node = self.change_node_input_dzhang(node)
      self.add_output_graph_node(node)
    return self.output_graph

#pw_batch_norm/Relu_eightbit_quantized:1

  def change_node_input_dzhang(self,node):
      new_node = node_def_pb2.NodeDef()
      new_node.CopyFrom(node)
      input_before_removal = node.input
      del new_node.input[:]
      for full_input_name in input_before_removal:
          if 'conv_ds_14/pw_batch_norm/Relu_eightbit_quantized:' not in full_input_name:
              if 'batch_norm/Relu_eightbit_quantized:' in full_input_name or 'pw_batch_norm/Relu_eightbit_quantized:' in full_input_name or 'convolution_eightbit_quantize_ExpandDims:' in full_input_name:
                  continue
              if 'convolution_eightbit_quantized_conv:' in full_input_name or 'depthwise_eightbit_quantized_depthwise:' in full_input_name or 'MatMul_eightbit_quantize_MobileNet/SpatialSqueeze:' in full_input_name or 'MatMul_eightbit_quantized_mat_mul:' in full_input_name:
                  continue
    ...
    # origine "remove_redundant_quantization"
  def remove_redundant_quantization_back(self, old_graph):
    """Removes unneeded pairs of quantize/dequantize ops from the graph.
...
def main(unused_args):
  ...
  # read 32bit_to_8bit_parameter

  parameter_file = open(FLAGS.parameter_file_name,"r");
  file_content = parameter_file.readlines()  
  for tem in file_content:
      value_list = tem.split(":")
      if value_list[0] == "quantize_parameter":
          global quantized_multiplier_arry
          save_to_global_arry(quantized_multiplier_arry, value_list)
      if value_list[0] == "right_shift":
          global right_shift_arry
          save_to_global_arry(right_shift_arry, value_list)
      if value_list[0] == "offset_input":
          global offset_input_arry
          save_to_global_arry(offset_input_arry, value_list)
      if value_list[0] == "offset_filter":
          global offset_filter_arry
          save_to_global_arry(offset_filter_arry, value_list)
      if value_list[0] == "offset_output":
          global offset_output_arry
          save_to_global_arry(offset_output_arry, value_list)
      if value_list[0] == "bias_add_offset_input1":
          global bias_add_offset_input1
          save_to_global_arry(bias_add_offset_input1, value_list)
      if value_list[0] == "bias_add_offset_input2":
          global bias_add_offset_input2
          save_to_global_arry(bias_add_offset_input2, value_list)
      if value_list[0] == "bias_add_multiplier_input1":
          global bias_add_multiplier_input1
          save_to_global_arry(bias_add_multiplier_input1, value_list)
      if value_list[0] == "bias_add_shift_input1":
          global bias_add_shift_input1
          save_to_global_arry(bias_add_shift_input1, value_list)
      if value_list[0] == "bias_add_multiplier_input2":
          global bias_add_multiplier_input2
          save_to_global_arry(bias_add_multiplier_input2, value_list)
      if value_list[0] == "bias_add_shift_input2":
          global bias_add_shift_input2
          save_to_global_arry(bias_add_shift_input2, value_list)
      if value_list[0] == "relue_arry":
          global relu_arry
          save_to_global_arry(relu_arry, value_list)
      if value_list[0] == "fc_bias_add_min":
          global fc_bias_add_min
          fc_bias_add_min = float(value_list[-1])
      if value_list[0] == "fc_bias_add_max":
          global fc_bias_add_max
          fc_bias_add_max = float(value_list[-1])
