...
def fold_batch_norms(input_graph_def):
  ...
  for node in input_graph_def.node:
    ...
    if name_list[-2] != "batchnorm" and not is_relu:
        continue
    nodes_to_skip[node.name] = True
    switch_op = node_from_map(input_node_map, node.input[0])
    if switch_op.op == "BiasAdd":
        old_ops["biasadd_op"] = switch_op
        nodes_to_skip[switch_op.name] = True
        bias_op = node_from_map(input_node_map, switch_op.input[1])
        if bias_op.op != "Const":
            tf_logging.warning("Didn't find expected bias Constant input to '%s',"
                    " found %s instead. Maybe because freeze_graph wasn't"
                    " run first?" % (switch_op, bias_op))
            continue
        bias = values_from_const(bias_op)
        nodes_to_skip[bias_op.name] = True
        value_consts["bias"] = bias
        old_ops["bias_op"] = bias_op
        conv_op = node_from_map(input_node_map, switch_op.input[0])
        old_ops["conv_op"] = conv_op
    ...
    if switch_op.op == "Const":
        const_split_pattern = re.compile(r'/')
        const_name_list = const_split_pattern.split(switch_op.name)
        if const_name_list[-1] == "moving_variance":
            moving_variance = values_from_const(switch_op)
            old_ops["variance_op"] = switch_op
            value_consts["moving_variance"] = moving_variance
            nodes_to_skip[switch_op.name] = True
        if const_name_list[-1] == "moving_mean":
            moving_mean = values_from_const(switch_op)
            old_ops["mean_op"] = switch_op
            value_consts["moving_mean"] = moving_mean
            nodes_to_skip[switch_op.name] = True
        if const_name_list[-1] == "beta":
            beta = values_from_const(switch_op)
            old_ops["beta_op"] = switch_op
            nodes_to_skip[switch_op.name] = True
            value_consts["beta"] = beta
    if switch_op.op == "Add" and not is_relu:
        y_op = node_from_map(input_node_map, switch_op.input[1])
        if y_op.op != "Const":
            tf_logging.warning("Didn't find expected y_op Constant input to '%s',"
                    "found %s instead. Maybe because freeze_graph wasn't"
                    " run first?" % (switch_op, y_op))
            continue
        y_value = values_from_const(y_op)
        old_ops["y_op"] = y_op
        nodes_to_skip[y_op.name] = True
        value_consts["y_value"] = y_value
    if not is_relu:
        continue
    else:
        is_relu = False
        scale_value = (1.0 / np.vectorize(math.sqrt)(value_consts["moving_variance"] + value_consts["y_value"]))
        ...
        scaled_weights_op.name = old_ops["weights_op"].name
        scaled_weights_op.attr["dtype"].CopyFrom(old_ops["weights_op"].attr["dtype"])
        ...
        offset_op.op = "Const"
        offset_op.name = old_ops["bias_op"].name
        offset_op.attr["dtype"].CopyFrom(old_ops["bias_op"].attr["dtype"])
        offset_op.attr["value"].CopyFrom(
            attr_value_pb2.AttrValue(tensor=tensor_util.make_tensor_proto(
                offset_value, value_consts["bias"].dtype.type, offset_value.shape)))
        bias_add_op = node_def_pb2.NodeDef()
        bias_add_op.op = "BiasAdd"
        bias_add_op.name = old_ops["biasadd_op"].name
        bias_add_op.attr["T"].CopyFrom(old_ops["conv_op"].attr["T"])
        bias_add_op.input.extend([new_conv_op.name, offset_op.name])
        relu_op = node_def_pb2.NodeDef()
        relu_op.op = "Relu"
        relu_op.name = old_ops["relu"].name
        relu_op.attr["T"].CopyFrom(old_ops["relu"].attr["T"])
        relu_op.input.extend([bias_add_op.name])
        new_ops.extend([scaled_weights_op, new_conv_op, offset_op, bias_add_op, relu_op])
        nodes_to_skip[old_ops["relu"].name] = True
        value_consts = {}
